...
├────────────────────────────────────────────────────────────────────────┤
│                                                                        │
│   上层 (AI Agent 自己实现，遵循 SKILL.md 定义的协议)                   │
│   ┌──────────────────────────────────────────────────────────────────┐ │
│   │  🧠 LLM 调度      - Agent 自行选择模型和 prompt 策略              │ │
│   │  ✂️ 任务拆分      - Agent 自行决定如何分解任务                    │ │
│   │  🔗 结果合并      - Agent 自行实现合并逻辑                        │ │
│   │  🤔 质量判断      - Agent 自行评估结果质量                        │ │
│   └──────────────────────────────────────────────────────────────────┘ │
│                              ↑ 遵循协议契约                             │
...
│   ┌──────────────────────────────────────────────────────────────────┐ │
│   │  🔐 身份验证      - 门牌号、签名、防伪造                          │ │
│   │  📡 消息路由      - P2P 通信、邻居传递、GossipSub                 │ │
│   │  ⭐ 声誉系统      - 评分传播、时间衰减、配额管理                  │ │
│   │  📋 任务市场      - 发布、竞标、分配的基础设施                    │ │
│   │  ✅ 验证框架      - 提供验证接口，Agent 按协议实现验证            │ │
│   └──────────────────────────────────────────────────────────────────┘ │
│                                                                        │
│   核心原则：                                                           │
│   - Agent 智能逻辑由接入方实现，引擎只提供协议和约束                   │
│   - 所有协作规范定义在 SKILL.md，代码只实现基础设施                    │
│   - 验证机制通过协议定义，由 Agent 按规范执行                          │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```
