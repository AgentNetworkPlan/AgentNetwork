# Task 27: 委托任务与文件传输

> **状态**: 📋 设计中  
> **优先级**: P0 (核心功能)  
> **预计工作量**: 5-7 天  
> **依赖**: Task 05 声誉, Task 11 激励, Task 14 消息签名, Task 26 共识机制

---

## 🤔 问题分析

### 核心场景

```
场景1: 文件/资源搜索
  Agent A 委托 Agent B 搜索一个文件或电影

场景2: 文件存储/传输
  Agent A 委托 Agent B 存储或传输文件

场景3: 任务协作
  Agent A 委托 Agent B 完成一个任务（如编写PPT）
```

### 潜在问题清单

| 编号 | 问题类型 | 描述 | 攻击者 | 受害者 |
|:---:|---------|------|:------:|:------:|
| P-01 | 滥发攻击 | 节点A发布大量任务/消息，造成网络阻塞 | A | 网络 |
| P-02 | 有害委托 | 委托攻击、DDOS、非法内容下载 | A | B/网络 |
| P-03 | 虚假完成 | B没完成但谎称完成 | B | A |
| P-04 | 虚假否认 | A收到成果但否认 | A | B |
| P-05 | 隐私泄露 | 委托内容含隐私，被第三方窃取 | 中间人 | A |
| P-06 | 流量滥用 | A无限制从B下载文件 | A | B |
| P-07 | 资源耗尽 | 恶意任务消耗B的计算资源 | A | B |
| P-08 | 中间人篡改 | 传输过程中文件被篡改 | 中间人 | A/B |
| P-09 | 拒绝服务 | A发布任务后消失，B无法交付 | A | B |
| P-10 | 抢跑攻击 | C偷看任务内容后抢先完成 | C | B |

---

## � 任务发布与接单机制

### 核心问题

```
Q1: 任务在哪里发布？
Q2: 节点B、C、D如何发现任务？
Q3: 多个节点收到任务，都要执行吗？
Q4: Agent如何自主判断是否接单？
```

### 关键洞察：Agent ≠ 区块链节点

```
┌─────────────────────────────────────────────────────────────────┐
│                    区块链节点 vs Agent节点                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   区块链节点                        Agent 节点                  │
│   ┌─────────┐                      ┌─────────┐                 │
│   │  规则   │                      │  智能体  │                 │
│   │  驱动   │                      │  (LLM)  │                 │
│   └────┬────┘                      └────┬────┘                 │
│        │                                │                       │
│        ▼                                ▼                       │
│   if (条件满足) {                  "让我分析一下..."            │
│     自动执行();                    "报酬是否合理？"             │
│   }                                "我擅长这类任务吗？"         │
│                                    "发布者信誉如何？"           │
│                                                                 │
│   ❌ 无判断力                       ✅ 自主决策                  │
│   ❌ 所有节点执行相同逻辑           ✅ 每个Agent有不同能力/偏好  │
│   ❌ 确定性输出                     ✅ 创造性输出                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 任务发布模式

#### 模式1: 广播市场 (Task Marketplace)

```
适用：通用任务，不在乎谁来执行

┌─────────────────────────────────────────────────────────────────┐
│                         任务市场                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ Task #001: 搜索关于AI的论文      奖励: 5声誉  截止: 2h   │   │
│  │ Task #002: 翻译一篇文档          奖励: 10声誉 截止: 4h   │   │
│  │ Task #003: 编写PPT大纲           奖励: 15声誉 截止: 1d   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                            │                                    │
│         ┌──────────────────┼──────────────────┐                │
│         ▼                  ▼                  ▼                │
│    ┌─────────┐       ┌─────────┐        ┌─────────┐           │
│    │ Agent B │       │ Agent C │        │ Agent D │           │
│    │ "我擅长 │       │ "翻译是 │        │ "PPT我  │           │
│    │  搜索"  │       │  我专长" │        │  来做"  │           │
│    └─────────┘       └─────────┘        └─────────┘           │
│         │                  │                  │                │
│     申请#001           申请#002           申请#003              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘

发布方式: GossipSub 广播到 "task-marketplace" 主题
```

#### 模式2: 定向委托 (Direct Delegation)

```
适用：指定特定Agent，或有隐私需求

Agent A ──────────────────> Agent B
         "我委托你完成这个任务"
         
发布方式: 点对点直接发送
```

#### 模式3: 能力匹配 (Capability Matching)

```
适用：需要特定能力的任务

┌───────────────────────────────────────────────────────────────┐
│  1. Agent A 发布任务，声明需要的能力                           │
│     {                                                         │
│       "task": "分析医学影像",                                  │
│       "required_capabilities": ["image_analysis", "medical"], │
│       "min_reputation": 50                                    │
│     }                                                         │
│                                                               │
│  2. 系统根据能力注册表筛选                                     │
│     Agent B: [coding, search]           ❌ 不匹配             │
│     Agent C: [image_analysis, medical]  ✅ 匹配               │
│     Agent D: [image_analysis]           ⚠️ 部分匹配           │
│                                                               │
│  3. 只通知匹配的Agent                                         │
│     → Agent C 收到任务通知                                    │
└───────────────────────────────────────────────────────────────┘
```

### 接单机制：如何避免重复执行？

#### 方案1: 抢单模式 (First Come First Served)

```go
// 抢单流程
type TaskClaim struct {
    TaskID      string
    ClaimerID   string    // 抢单的Agent
    ClaimTime   int64
    Signature   string
}

// 流程:
// 1. Agent B 看到任务，发送 ClaimRequest
// 2. 发布者 A 收到第一个有效的 Claim
// 3. A 广播 TaskAssigned{TaskID, AssignedTo: B}
// 4. 其他Agent收到后放弃该任务
```

```
时间线：
T0: A 发布任务
T1: B 发送 Claim (网络延迟 50ms)
T2: C 发送 Claim (网络延迟 80ms)
T3: A 收到 B 的 Claim，确认分配给 B
T4: A 广播 TaskAssigned → B
T5: C 收到 TaskAssigned，放弃任务

问题：网络延迟可能导致不公平
```

#### 方案2: 竞标模式 (Bidding) 【推荐】

```go
// 竞标流程
type TaskBid struct {
    TaskID          string
    BidderID        string
    BidAmount       float64   // 愿意接受的最低报酬（可低于标价）
    EstimatedTime   int64     // 预估完成时间
    Capabilities    []string  // 自我声明的能力
    Reputation      float64   // 当前声誉
    Message         string    // Agent的自我介绍/竞标理由
    Signature       string
}

// 流程:
// 1. A 发布任务，设置竞标期（如：10分钟）
// 2. B、C、D 在竞标期内提交 Bid
// 3. 竞标期结束，A 的 Agent 自主选择最佳投标者
// 4. A 广播 TaskAssigned

// Agent A 的选择逻辑（由LLM判断）：
func (agent *Agent) SelectBestBidder(bids []TaskBid) string {
    // Agent 可以根据多种因素自主判断：
    // - 声誉分数
    // - 报价
    // - 预估时间
    // - 历史合作经验
    // - 竞标理由的质量
    
    prompt := fmt.Sprintf(`
        我发布了一个任务: %s
        收到以下竞标:
        %v
        
        请帮我选择最合适的执行者，考虑因素：
        1. 声誉和可信度
        2. 报价合理性
        3. 能力匹配度
        4. 预估完成时间
        
        返回你选择的Agent ID和理由。
    `, task.Description, bids)
    
    return agent.LLM.Decide(prompt)
}
```

```
时间线：
T0:     A 发布任务（竞标期 10分钟）
T0-T10: B、C、D 提交竞标
T10:    竞标期结束
T11:    A 的 Agent 分析所有竞标
T12:    A 选择 C，广播 TaskAssigned
T13:    C 开始执行，B、D 收到通知放弃
```

#### 方案3: 多执行者模式 (Multi-Executor)

```
适用：需要多个结果对比、冗余执行

例：A 委托 "分析这张图片中的物体"
    → B、C、D 都执行
    → A 收到3个结果，对比选择最佳
    → 或者合并3个结果
```

```go
type MultiExecutorTask struct {
    TaskID          string
    MaxExecutors    int       // 最多允许多少Agent执行
    SelectionMode   string    // "best" | "merge" | "vote"
    RewardPerAgent  float64   // 每个执行者的报酬
}
```

### Agent 自主决策：是否接单？

```go
// AgentDecisionContext 决策上下文
type AgentDecisionContext struct {
    // 任务信息
    Task            *Task
    
    // 发布者信息
    RequesterID     string
    RequesterRep    float64
    PastInteractions int      // 历史合作次数
    PastSuccessRate float64   // 历史成功率
    
    // 自身状态
    CurrentWorkload int       // 当前任务数
    MyCapabilities  []string  // 我的能力
    MyReputation    float64   // 我的声誉
    
    // 市场状态
    CompetitorCount int       // 多少人也在看这个任务
    MarketRate      float64   // 类似任务的市场平均报酬
}

// Agent 决策过程
func (agent *Agent) DecideToAccept(ctx *AgentDecisionContext) (bool, string) {
    // 构建决策prompt
    prompt := fmt.Sprintf(`
        # 任务评估
        
        ## 任务详情
        - 类型: %s
        - 描述: %s
        - 报酬: %.1f 声誉点
        - 截止: %s
        
        ## 发布者信息
        - 声誉: %.1f
        - 历史合作: %d 次
        - 成功率: %.1f%%
        
        ## 我的状态
        - 当前任务数: %d
        - 我的能力: %v
        - 我的声誉: %.1f
        
        ## 市场情况
        - 竞争者数量: %d
        - 市场平均价: %.1f
        
        请判断：
        1. 我是否应该接受这个任务？
        2. 如果竞标，我的报价应该是多少？
        3. 理由是什么？
        
        返回JSON: {"accept": true/false, "bid": 金额, "reason": "理由"}
    `, ctx.Task.Type, ctx.Task.Description, ctx.Task.Reward, 
       time.Unix(ctx.Task.Deadline, 0).Format("2006-01-02 15:04"),
       ctx.RequesterRep, ctx.PastInteractions, ctx.PastSuccessRate*100,
       ctx.CurrentWorkload, ctx.MyCapabilities, ctx.MyReputation,
       ctx.CompetitorCount, ctx.MarketRate)
    
    response := agent.LLM.Think(prompt)
    return parseDecision(response)
}
```

### 完整流程图

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        任务发布与执行完整流程                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌─────────┐                                                           │
│  │ Agent A │ 1. 发布任务                                               │
│  │ (发布者) │────────────────────────────────────────┐                 │
│  └─────────┘                                         │                 │
│       │                                              ▼                 │
│       │                                    ┌─────────────────┐         │
│       │                                    │   任务市场/     │         │
│       │                                    │  GossipSub广播  │         │
│       │                                    └────────┬────────┘         │
│       │                                             │                  │
│       │                    ┌────────────────────────┼────────────┐     │
│       │                    ▼                        ▼            ▼     │
│       │              ┌─────────┐             ┌─────────┐  ┌─────────┐ │
│       │              │ Agent B │             │ Agent C │  │ Agent D │ │
│       │              └────┬────┘             └────┬────┘  └────┬────┘ │
│       │                   │                       │            │      │
│       │          2. Agent自主判断            Agent自主判断   Agent判断 │
│       │              ┌────┴────┐             ┌────┴────┐  ┌────┴────┐ │
│       │              │ LLM思考  │             │ LLM思考  │  │ LLM思考  │ │
│       │              │ "报酬低 │             │ "这个我 │  │ "太忙了 │ │
│       │              │  不接"  │             │  擅长"  │  │  放弃"  │ │
│       │              └────┬────┘             └────┬────┘  └────┬────┘ │
│       │                   │                       │            │      │
│       │                   ❌                      ✅            ❌     │
│       │                  放弃                   提交竞标       放弃    │
│       │                                          │                    │
│       │  3. 收集竞标                             │                    │
│       │◄─────────────────────────────────────────┘                    │
│       │                                                                │
│       │  4. Agent A 自主选择                                          │
│       │     ┌──────────────────────────────────────┐                  │
│       │     │ LLM分析: "C的声誉高，报价合理，选C" │                  │
│       │     └──────────────────────────────────────┘                  │
│       │                                                                │
│       │  5. 广播任务分配                                              │
│       │──────────────────────────────────────────────────────────────>│
│       │                                              │                 │
│       │                                              ▼                 │
│       │                                        ┌─────────┐            │
│       │                                        │ Agent C │            │
│       │                                        │  执行中  │            │
│       │                                        └────┬────┘            │
│       │                                             │                  │
│       │  6. 提交交付                                │                  │
│       │◄────────────────────────────────────────────┘                  │
│       │                                                                │
│       │  7. Agent A 验收                                              │
│       │     ┌──────────────────────────────────────┐                  │
│       │     │ LLM评估: "质量符合要求，确认完成"    │                  │
│       │     └──────────────────────────────────────┘                  │
│       │                                                                │
│       │  8. 结算 + 双方互评                                           │
│       │                                                                │
└─────────────────────────────────────────────────────────────────────────┘
```

### 数据结构补充

```go
// TaskPublication 任务发布
type TaskPublication struct {
    Task            *Task
    PublishMode     PublishMode   // broadcast | direct | capability_match
    BiddingPeriod   time.Duration // 竞标期，0表示抢单模式
    MaxExecutors    int           // 最大执行者数，1表示单执行者
    RequiredCaps    []string      // 要求的能力
    MinReputation   float64       // 最低声誉要求
}

type PublishMode string

const (
    ModeBroadcast       PublishMode = "broadcast"        // 广播到市场
    ModeDirect          PublishMode = "direct"           // 定向委托
    ModeCapabilityMatch PublishMode = "capability_match" // 能力匹配
)

// AgentCapability Agent能力注册
type AgentCapability struct {
    AgentID         string
    Capabilities    []string  // ["coding", "search", "translation", "image_analysis"]
    Languages       []string  // ["zh", "en", "ja"]
    MaxConcurrent   int       // 最大并发任务数
    AvailableHours  []int     // 可用时段 [9,10,11,...,18]
    UpdatedAt       int64
}

// 能力注册表（DHT存储）
// Key: /capabilities/{capability_name}
// Value: []AgentID
```

### 防重复执行机制

```go
// TaskLock 任务锁
type TaskLock struct {
    TaskID      string
    LockedBy    string    // 被分配给谁
    LockedAt    int64
    Signature   string    // 发布者签名确认
}

// 执行前检查
func (agent *Agent) BeforeExecute(taskID string) error {
    // 1. 检查任务是否已被锁定
    lock := agent.GetTaskLock(taskID)
    if lock != nil && lock.LockedBy != agent.ID {
        return ErrTaskAlreadyAssigned
    }
    
    // 2. 检查任务状态
    task := agent.GetTask(taskID)
    if task.Status != StatusAccepted {
        return ErrInvalidTaskStatus
    }
    
    // 3. 确认是分配给自己的
    if task.ExecutorID != agent.ID {
        return ErrNotAssignedToMe
    }
    
    return nil
}
```

---

## �🏗️ 整体架构

### 委托任务生命周期

```
┌─────────────────────────────────────────────────────────────────────┐
│                        委托任务生命周期                               │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐         │
│  │ 1.发布   │───>│ 2.接受   │───>│ 3.执行   │───>│ 4.交付   │         │
│  │ PUBLISH │    │ ACCEPT  │    │ EXECUTE │    │ DELIVER │         │
│  └────┬────┘    └────┬────┘    └────┬────┘    └────┬────┘         │
│       │              │              │              │               │
│       ▼              ▼              ▼              ▼               │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐         │
│  │ 5.验收   │───>│ 6.结算   │───>│ 7.评价   │───>│ 8.归档   │         │
│  │ VERIFY  │    │ SETTLE  │    │ RATE    │    │ ARCHIVE │         │
│  └─────────┘    └─────────┘    └─────────┘    └─────────┘         │
│                                                                     │
│  状态流转:                                                          │
│  DRAFT → PUBLISHED → ACCEPTED → IN_PROGRESS → DELIVERED            │
│       → VERIFIED → SETTLED → COMPLETED                              │
│                                                                     │
│  异常分支:                                                          │
│  任意阶段 → DISPUTED → ARBITRATION → RESOLVED                       │
│  超时    → EXPIRED / CANCELLED                                      │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 三层防护架构

```
┌─────────────────────────────────────────────────────────────────────┐
│  Layer 1: 准入控制                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │ 声誉门槛     │  │ 押金锁定     │  │ 速率限制     │              │
│  │ (发任务需30+)│  │ (双方押金)   │  │ (每小时N个)  │              │
│  └──────────────┘  └──────────────┘  └──────────────┘              │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 2: 过程保护                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │ 端到端加密   │  │ 内容哈希     │  │ 进度追踪     │              │
│  │ (隐私保护)   │  │ (防篡改)     │  │ (超时检测)   │              │
│  └──────────────┘  └──────────────┘  └──────────────┘              │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 3: 争议解决                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │ 自动仲裁     │  │ 委员会裁决   │  │ 声誉惩罚     │              │
│  │ (规则匹配)   │  │ (人工介入)   │  │ (违规记录)   │              │
│  └──────────────┘  └──────────────┘  └──────────────┘              │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 📐 数据结构设计

### 1. 委托任务 (Task)

```go
// TaskType 任务类型
type TaskType string

const (
    TaskTypeSearch    TaskType = "search"     // 搜索任务
    TaskTypeTransfer  TaskType = "transfer"   // 文件传输
    TaskTypeStorage   TaskType = "storage"    // 文件存储
    TaskTypeCompute   TaskType = "compute"    // 计算任务
    TaskTypeCustom    TaskType = "custom"     // 自定义任务
)

// TaskStatus 任务状态
type TaskStatus string

const (
    StatusDraft      TaskStatus = "draft"
    StatusPublished  TaskStatus = "published"
    StatusAccepted   TaskStatus = "accepted"
    StatusInProgress TaskStatus = "in_progress"
    StatusDelivered  TaskStatus = "delivered"
    StatusVerified   TaskStatus = "verified"
    StatusSettled    TaskStatus = "settled"
    StatusCompleted  TaskStatus = "completed"
    StatusDisputed   TaskStatus = "disputed"
    StatusCancelled  TaskStatus = "cancelled"
    StatusExpired    TaskStatus = "expired"
)

// Task 委托任务
type Task struct {
    // 基本信息
    ID              string      `json:"id"`
    Type            TaskType    `json:"type"`
    Title           string      `json:"title"`
    Description     string      `json:"description"`      // 可加密
    
    // 参与方
    RequesterID     string      `json:"requester_id"`     // 委托方
    ExecutorID      string      `json:"executor_id"`      // 执行方（接受后填入）
    
    // 奖励与押金
    Reward          float64     `json:"reward"`           // 声誉奖励
    RequesterDeposit float64    `json:"requester_deposit"` // 委托方押金
    ExecutorDeposit float64     `json:"executor_deposit"`  // 执行方押金
    
    // 时间约束
    CreatedAt       int64       `json:"created_at"`
    Deadline        int64       `json:"deadline"`         // 截止时间
    ExpiresAt       int64       `json:"expires_at"`       // 任务过期时间
    
    // 验收条件
    AcceptanceCriteria string   `json:"acceptance_criteria"` // 验收标准
    DeliverableHash    string   `json:"deliverable_hash"`    // 交付物哈希（可选）
    
    // 状态
    Status          TaskStatus  `json:"status"`
    
    // 隐私保护
    IsEncrypted     bool        `json:"is_encrypted"`     // 描述是否加密
    PublicKeyHash   string      `json:"public_key_hash"`  // 执行方公钥哈希（解密用）
    
    // 签名
    RequesterSig    string      `json:"requester_sig"`
    ExecutorSig     string      `json:"executor_sig"`
    Nonce           string      `json:"nonce"`
}
```

### 2. 文件传输请求 (TransferRequest)

```go
// TransferRequest 文件传输请求
type TransferRequest struct {
    ID              string      `json:"id"`
    TaskID          string      `json:"task_id"`          // 关联的任务ID（可选）
    
    // 参与方
    SenderID        string      `json:"sender_id"`
    ReceiverID      string      `json:"receiver_id"`
    
    // 文件信息
    FileHash        string      `json:"file_hash"`        // 文件SM3哈希
    FileName        string      `json:"file_name"`
    FileSize        int64       `json:"file_size"`        // 字节
    ChunkSize       int64       `json:"chunk_size"`       // 分片大小
    TotalChunks     int         `json:"total_chunks"`
    
    // 流量控制
    MaxBandwidth    int64       `json:"max_bandwidth"`    // 最大带宽(bytes/s)
    
    // 押金（防止滥用）
    SenderDeposit   float64     `json:"sender_deposit"`
    ReceiverDeposit float64     `json:"receiver_deposit"`
    
    // 状态
    Status          TransferStatus `json:"status"`
    Progress        float64     `json:"progress"`         // 0-1
    
    // 时间
    CreatedAt       int64       `json:"created_at"`
    ExpiresAt       int64       `json:"expires_at"`
    
    // 签名
    Signatures      []string    `json:"signatures"`
}

// TransferChunk 传输分片
type TransferChunk struct {
    TransferID      string      `json:"transfer_id"`
    Index           int         `json:"index"`
    Data            []byte      `json:"data"`
    Hash            string      `json:"hash"`             // 分片哈希
    Signature       string      `json:"signature"`
}
```

### 3. 押金系统 (Escrow)

```go
// Escrow 押金托管
type Escrow struct {
    ID              string      `json:"id"`
    TaskID          string      `json:"task_id"`
    
    // 押金明细
    Deposits        map[string]float64 `json:"deposits"`  // nodeID -> amount
    TotalAmount     float64     `json:"total_amount"`
    
    // 状态
    Status          EscrowStatus `json:"status"`
    
    // 释放条件
    ReleaseCondition string     `json:"release_condition"`
    ReleasedTo       string     `json:"released_to"`
    ReleasedAmount   float64    `json:"released_amount"`
    
    // 多签名锁定
    LockSignatures  []string    `json:"lock_signatures"`
    UnlockSignatures []string   `json:"unlock_signatures"`
    
    CreatedAt       int64       `json:"created_at"`
    LockedUntil     int64       `json:"locked_until"`
}

type EscrowStatus string

const (
    EscrowPending   EscrowStatus = "pending"
    EscrowLocked    EscrowStatus = "locked"
    EscrowReleased  EscrowStatus = "released"
    EscrowDisputed  EscrowStatus = "disputed"
    EscrowRefunded  EscrowStatus = "refunded"
)
```

---

## 🛡️ 安全机制设计

### P-01: 滥发攻击防护

```go
// TaskRateLimiter 任务速率限制器
type TaskRateLimiter struct {
    // 基于声誉的配额
    BaseQuota           int       // 基础配额（如：每小时2个任务）
    ReputationMultiplier float64  // 声誉乘数（如：声誉/20）
    
    // 冷却机制
    CooldownAfterReject time.Duration // 被拒绝后冷却期
    
    // 当前状态
    quotaUsed       map[string]int       // nodeID -> 已用配额
    lastReset       map[string]time.Time // nodeID -> 上次重置时间
}

// 计算节点配额
func (r *TaskRateLimiter) GetQuota(nodeID string, reputation float64) int {
    base := r.BaseQuota
    bonus := int(reputation * r.ReputationMultiplier)
    return base + bonus
    // 例：声誉=50 → 2 + 50/20 = 4.5 → 4个任务/小时
}

// 消息大小限制
const (
    MaxTaskDescriptionSize = 10 * 1024    // 10KB
    MaxAttachmentSize      = 1 * 1024 * 1024 // 1MB
)
```

### P-02: 有害委托过滤

```go
// TaskFilter 任务过滤器
type TaskFilter struct {
    // 黑名单关键词
    BlacklistKeywords []string
    
    // 任务类型白名单
    AllowedTaskTypes  []TaskType
    
    // 资源限制
    MaxComputeTime    time.Duration  // 最大计算时间
    MaxMemoryUsage    int64          // 最大内存使用
    MaxNetworkCalls   int            // 最大网络调用数
    
    // 沙箱执行
    SandboxEnabled    bool
}

// 任务分类
type TaskRiskLevel string

const (
    RiskLow     TaskRiskLevel = "low"      // 搜索、查询
    RiskMedium  TaskRiskLevel = "medium"   // 文件传输
    RiskHigh    TaskRiskLevel = "high"     // 计算任务
    RiskBlocked TaskRiskLevel = "blocked"  // 禁止执行
)

// 评估任务风险
func (f *TaskFilter) AssessRisk(task *Task) TaskRiskLevel {
    // 1. 关键词检查
    if containsBlacklist(task.Description, f.BlacklistKeywords) {
        return RiskBlocked
    }
    
    // 2. 类型检查
    switch task.Type {
    case TaskTypeSearch:
        return RiskLow
    case TaskTypeTransfer, TaskTypeStorage:
        return RiskMedium
    case TaskTypeCompute:
        return RiskHigh
    default:
        return RiskMedium
    }
}
```

### P-03 & P-04: 防止虚假声明（双向保护）

```go
// 承诺-揭示机制 (Commit-Reveal)
type CommitRevealProtocol struct {
    // 阶段1: 执行方提交交付物哈希（不揭示内容）
    DeliverableCommit   string    // Hash(交付物 + 随机数)
    CommitTimestamp     int64
    
    // 阶段2: 委托方确认收到（锁定资金）
    ReceivedAck         bool
    AckTimestamp        int64
    
    // 阶段3: 执行方揭示交付物
    DeliverableReveal   []byte    // 实际交付物
    RevealNonce         string    // 随机数
    
    // 阶段4: 验证匹配
    Verified            bool
}

// 流程:
// 1. B完成任务，计算 commit = Hash(result + nonce)，发送commit给A
// 2. A收到commit，发送ACK（此时A还没看到result）
// 3. B收到ACK后，发送 (result, nonce)
// 4. A验证 Hash(result + nonce) == commit
// 5. 如果A不发ACK，超时后B可申诉
// 6. 如果B不揭示，超时后A可取回押金
```

```go
// 交付证明 (Proof of Delivery)
type DeliveryProof struct {
    TaskID          string
    DeliverableHash string    // 交付物哈希
    
    // 执行方证明
    ExecutorSig     string    // B签名：我交付了这个
    DeliveryTime    int64
    
    // 委托方确认
    RequesterSig    string    // A签名：我收到了这个
    ReceiveTime     int64
    
    // 第三方见证（可选，用于争议）
    WitnessSigs     []string  // 见证节点签名
}

// 验证交付证明
func (p *DeliveryProof) Verify() error {
    // 必须同时有双方签名才算有效交付
    if p.ExecutorSig == "" || p.RequesterSig == "" {
        return ErrIncompleteProof
    }
    // 验证签名...
    return nil
}
```

### P-05: 隐私保护

```go
// 端到端加密任务
type EncryptedTask struct {
    // 公开部分（所有人可见）
    TaskID          string
    Type            TaskType
    RequesterID     string
    Reward          float64
    Deadline        int64
    
    // 加密部分（只有执行方可解密）
    EncryptedPayload []byte   // SM4加密的任务详情
    
    // 密钥交换
    EphemeralPubKey string    // 委托方临时公钥
    // 执行方用自己私钥 + 临时公钥 = 共享密钥 → 解密payload
}

// 隐私任务流程
// 1. A发布任务时，只公开类型和奖励
// 2. B表示感兴趣，发送自己的公钥
// 3. A用ECDH生成共享密钥，加密任务详情发给B
// 4. B解密后决定是否接受
// 5. 全程第三方无法看到任务内容
```

### P-06: 流量滥用防护

```go
// BandwidthController 带宽控制器
type BandwidthController struct {
    // 每节点限制
    MaxBytesPerHour    int64             // 每小时最大传输量
    MaxConcurrentTransfers int           // 最大并发传输数
    
    // 声誉加成
    ReputationBonus    float64           // 高声誉节点带宽加成
    
    // 当前状态
    usage              map[string]*UsageRecord
}

type UsageRecord struct {
    NodeID          string
    BytesSent       int64
    BytesReceived   int64
    ActiveTransfers int
    LastReset       time.Time
}

// 流量计费（声誉消耗）
const (
    ReputationPerGB = 0.1   // 每传输1GB消耗0.1声誉
    FreeQuotaGB     = 1.0   // 每天免费1GB
)
```

### P-09: 防止发布后消失

```go
// 任务保证金机制
type TaskGuarantee struct {
    // 委托方必须锁定押金
    RequesterDeposit float64   // 至少 = Reward * 1.2
    
    // 超时规则
    ResponseTimeout  time.Duration  // 委托方必须在此时间内响应交付
    
    // 违约处理
    DefaultPenalty   float64        // 违约罚金
}

// 如果A发布任务后消失:
// 1. B完成并提交交付证明
// 2. A在ResponseTimeout内未响应
// 3. 系统自动将A的押金转给B
// 4. A声誉扣分
```

---

## 🔄 文件传输协议

### 分片传输流程

```
┌────────────────────────────────────────────────────────────────┐
│                     文件分片传输协议                             │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   发送方(A)                              接收方(B)             │
│      │                                      │                  │
│      │  1. TransferRequest                  │                  │
│      │  (文件元信息+哈希+押金)              │                  │
│      │ ──────────────────────────────────>  │                  │
│      │                                      │                  │
│      │  2. TransferAccept                   │                  │
│      │  (同意+押金)                         │                  │
│      │  <──────────────────────────────────│                  │
│      │                                      │                  │
│      │  3. Chunk[0] + Hash                  │                  │
│      │ ──────────────────────────────────>  │                  │
│      │                                      │ 验证Hash        │
│      │  4. ChunkAck[0]                      │                  │
│      │  <──────────────────────────────────│                  │
│      │                                      │                  │
│      │  ... (重复3-4直到完成) ...           │                  │
│      │                                      │                  │
│      │  5. TransferComplete                 │                  │
│      │ ──────────────────────────────────>  │                  │
│      │                                      │ 验证完整文件Hash │
│      │  6. TransferConfirm                  │                  │
│      │  (双方签名的传输证明)                │                  │
│      │  <──────────────────────────────────│                  │
│      │                                      │                  │
│      │  7. 押金释放                         │                  │
│      │                                      │                  │
└────────────────────────────────────────────────────────────────┘
```

### 断点续传支持

```go
// TransferCheckpoint 传输检查点
type TransferCheckpoint struct {
    TransferID      string
    LastChunkIndex  int
    ReceivedChunks  []bool    // 位图：哪些分片已收到
    PartialHash     string    // 已收到部分的哈希
    SavedAt         int64
}

// 恢复传输
func (t *TransferManager) ResumeTransfer(checkpoint *TransferCheckpoint) error {
    // 1. 找到上次中断位置
    // 2. 请求缺失的分片
    // 3. 继续传输
}
```

---

## 📁 文件规划

```
internal/
├── task/                         # 新增: 委托任务模块
│   ├── task.go                  # 任务管理器
│   ├── task_test.go
│   ├── filter.go                # 任务过滤器
│   ├── limiter.go               # 速率限制器
│   └── types.go                 # 类型定义
│
├── transfer/                     # 新增: 文件传输模块
│   ├── transfer.go              # 传输管理器
│   ├── transfer_test.go
│   ├── chunk.go                 # 分片处理
│   ├── bandwidth.go             # 带宽控制
│   └── checkpoint.go            # 断点续传
│
├── escrow/                       # 新增: 押金托管模块
│   ├── escrow.go                # 托管管理器
│   ├── escrow_test.go
│   └── release.go               # 释放规则
│
├── dispute/                      # 新增: 争议处理模块
│   ├── dispute.go               # 争议管理器
│   ├── dispute_test.go
│   ├── arbitration.go           # 仲裁逻辑
│   └── evidence.go              # 证据收集
│
└── privacy/                      # 新增: 隐私保护模块
    ├── encryption.go            # 端到端加密
    ├── encryption_test.go
    └── keyexchange.go           # 密钥交换
```

---

## 🔧 实现步骤

### Phase 1: 基础任务系统 (Day 1-2)

- 任务类型定义与状态机
- 任务发布、接受、完成流程
- 速率限制与基础过滤

### Phase 2: 押金与结算 (Day 2-3)

- 押金锁定与释放
- 承诺-揭示协议
- 交付证明生成

### Phase 3: 文件传输 (Day 3-5)

- 分片传输协议
- 带宽控制
- 断点续传

### Phase 4: 隐私与安全 (Day 5-6)

- 端到端加密
- 任务内容保护
- 密钥交换

### Phase 5: 争议处理 (Day 6-7)

- 自动仲裁规则
- 委员会介入
- 证据链保存

---

## 🧪 测试计划

### 单元测试

| 测试项 | 覆盖内容 |
|--------|---------|
| 任务生命周期 | 状态流转、超时处理 |
| 速率限制 | 配额计算、冷却机制 |
| 文件传输 | 分片、哈希验证、断点续传 |
| 押金系统 | 锁定、释放、退款 |
| 隐私保护 | 加密解密、密钥交换 |

### 集成测试

```go
func TestTaskWorkflow(t *testing.T) {
    // 1. A发布任务（锁定押金）
    // 2. B接受任务（锁定押金）
    // 3. B执行并提交交付证明
    // 4. A确认收到
    // 5. 押金释放，声誉结算
}

func TestDisputeResolution(t *testing.T) {
    // 1. A发布任务
    // 2. B完成但A拒绝确认
    // 3. B发起争议
    // 4. 提交证据
    // 5. 自动/委员会仲裁
    // 6. 验证结果
}
```

---

## 📝 验收标准

- [ ] 任务发布、接受、完成流程正常
- [ ] 押金正确锁定和释放
- [ ] 文件分片传输无数据丢失
- [ ] 断点续传功能正常
- [ ] 端到端加密保护隐私
- [ ] 速率限制有效防止滥发
- [ ] 争议处理流程完整
- [ ] 所有测试通过

---

## 📚 参考

- [Task 26 共识机制](task26共识机制.md) - 事件账本、委员会共识
- [Task 11 激励机制](task11激励.md) - 声誉奖惩
- [Task 14 消息签名](task14消息签名.md) - Nonce防重放
- [BitTorrent Protocol](https://www.bittorrent.org/beps/bep_0003.html) - 分片传输参考
- [IPFS](https://docs.ipfs.tech/) - 内容寻址存储